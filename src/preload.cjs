...
contextBridge.exposeInMainWorld('noor', {
  ...
  providers: {
    ...
    tokenInConnect: (key) => invoke('provider:tokenin-connect', key),
    tokenInDisconnect: () => invoke('provider:tokenin-disconnect'),
    tokenInRefresh: () => invoke('provider:tokenin-refresh'),
